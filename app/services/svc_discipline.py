...
class DisciplineService:
    ...
    def list_disciplines(self, search: str | None, page: int, page_size: int) -> PaginatedResponse[DisciplineSummary]:
        ...
        return PaginatedResponse[DisciplineSummary].build(
            items=[DisciplineSummary.from_model(item) for item in items],
            total=total,
            page=page,
            page_size=page_size,
        )
